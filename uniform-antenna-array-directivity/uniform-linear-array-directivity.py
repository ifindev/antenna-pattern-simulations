...
N=10
d=2*la #distance between array element follow wavelength
delta=0
...
ax.set_zlabel("D (dB)")
ax.set_title('Directivity ULA (d = 2$\lambda$)',size=20)
...
levels = [i for i in range(-60,80,10)]
...
ax.set_title('Directivity ULA (d = 2$\lambda$)',size=20)
ax.set_xlabel('$\Theta$ (degree)',size=15)
ax.set_ylabel('$\phi$ (degree)',size=15)
legend(["$D_0$ = %.3f dB" %a])
